...
- 在上采样时**使用了比较粗糙的最近邻插值，使得高层的语义信息不一定能有效传播；**
...
- **空洞卷积：**本身可以控制不同大小的感受野，也即可以处理多尺度（网格效应：[1, 3, 5, 1, 3, 5]） -> **TridentNet**
...
- 解决不同层级上可学习样本数量不平衡问题(避免了某个head因为样本不充分而导致学习的参数不对的情况) 
- 多尺度语义不变性使共享head可行
- 少参数提升精度



共享head不用BN层的原因在于**训练和测试阶段BN层操作行为不一致**！在训练过程中，所有BN层的均值和方差会以移动平均方式不断累加，由于head共享，移动均值和方差会在不同特征层间**跳动**，导致训练结束后得到**全期移动均值和方差**不稳定，而在测试时，BN层参数是不进行更新的，共享head测试所用的均值和方差是同一套全期移动均值和方差，这种情况下推理的效果往往会比较差导致模型失效；

因此在使用共享head时，切忌使用BN，如果真的需要归一化处理，**可采用GN进行替代**，因为GN在训练和测试都是直接基于特征图计算的，没有参数的保存，所以训练和测试保持行为一致
