...
sigmoid函数 $\sigma(z)$ 的导数sigmoid $\sigma^{\prime}(z)$ 在输出接近 0 和 1 的时候是非常 小的，故导致在使用最小均方差Loss时，模型参数w会学习的非常慢；而使用交叉商Loss则没有这 个问题。为了更快的学习速度，分类问题一般采用交叉商损失函数。
...
**PR曲线下面积就称为 AP**；	**纵坐标：P	横坐标：R**，实际上，AP是在0到1之间的所有召回值上平均的精度

```
mAP: 各类别AP的平均值

AP: PR曲线下面积(AP计算：在VOC2010及以后，需要针对每一个不同的Recall值（包括0和1），选取其大于等于这些Recall值时的Precision最大值，然后计算PR曲线下面积作为AP值)
...
TP: IoU>thd的检测框数量（同一Ground Truth只计算一次）

FP: IoU<=thd的检测框，或者是检测到同一个GT的多余检测框的数量
...
AP模拟：https://github.com/Inmessionant/Deep-Learning-Tricks/blob/main/AP.pdf



![img](https://github.com/rafaelpadilla/Object-Detection-Metrics/raw/master/aux_images/samples_1_v2.png)

有7张图像，其中有15个以绿色边界框表示的gt对象和24个以红色边界框表示的检测到的对象。每个检测到的对象都有一个置信度，并由字母（A，B，...，Y）标识。

下表显示了边界框及其相应的置信度。最后一列将检测结果标识为TP或FP。在此示例中，如果IOU为>=30％，则考虑TP ，否则为FP。通过查看上面的图像，我们可以大致分辨出检测的是TP还是FP(TP/FP是我们大概看出来的，实际计算中需要计算IoU卡阈值)，在某些图像中，有多个检测项与gt重叠（图像2、3、4、5、6和7）。对于那些情况，第一个检测被认为是TP，而其他检测是FP。PASCAL VOC 2012度量标准应用了此规则：“例如，将单个对象的5次检测（TP）计为1次正确检测和4次错误检测”

![img](https://github.com/rafaelpadilla/Object-Detection-Metrics/raw/master/aux_images/table_1_v2.png)



**通过计算累计的TP或FP检测的精度和召回值来绘制Precision x召回曲线**

为此，首先我们需要按照其置信度对检测进行排序，然后针对每个累积的检测计算精度和召回率，如下表所示（请注意，对于召回计算，分母项（“ Acc TP + Acc FN”或“所有基本事实”）恒定为15，因为无论检测到什么，GT盒都是恒定的）：

![img](https://github.com/rafaelpadilla/Object-Detection-Metrics/raw/master/aux_images/table_2_v2.png)

第二行的示例计算（图7）：精度= TP /（TP + FP）= 1/2 = 0.5且调用= TP /（TP + FN）= 1/15 = 0.066

绘制精度和查全率值，我们具有以下“P—R”曲线：

![img](https://github.com/rafaelpadilla/Object-Detection-Metrics/raw/master/aux_images/precision_recall_example_1_v2.png)

**计算在所有点上执行的插值**

通过插值所有点，平均精度（AP）可以解释为Precision x Recall曲线的近似AUC。目的是减少曲线中摆动的影响。通过查看从最高（0.4666）到0（从右到左的图）的召回率，我们还可以从视觉上获得内插的精度点，并且随着我们降低召回率，我们收集了最高的精度值如下图所示：

![img](https://github.com/rafaelpadilla/Object-Detection-Metrics/raw/master/aux_images/interpolated_precision_v2.png)

查看上图，我们可以将AUC划分为4个区域（A1，A2，A3和A4）：

![img](https://github.com/rafaelpadilla/Object-Detection-Metrics/raw/master/aux_images/interpolated_precision-AUC_v2.png)

计算总面积，我们得到AP：

$A P=A 1+A 2+A 3+A 4$
with:
$A 1=(0.0666-0) \times 1=0.0666$
$A 2=(0.1333-0.0666) \times 0.6666=0.04446222$
$A 3=(0.4-0.1333) \times 0.4285=0.11428095$
$A 4=(0.4666-0.4) \times 0.3043=0.02026638$
$A P=0.0666+0.04446222+0.11428095+0.02026638$
$A P=0.24560955$
$A P=24.56 \%$
