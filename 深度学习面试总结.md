...
**有7张图像，其中有15个以绿色边界框表示的gt对象和24个以红色边界框表示的检测到的对象；每个检测到的对象都有一个置信度，并由（A，B，...，Y）标识**
...
下表显示了边界框及其相应的置信度：最后一列将检测结果标识为TP或FP。在此示例中，如果IOU为>=30％，则**考虑TP** ，否则为FP。通过查看上面的图像，我们可以大致分辨出检测的是TP还是FP，**在某些图像中，有多个检测项与gt重叠，对于那些情况，第一个检测被认为是TP，而其他检测是FP**：PASCAL VOC 2012度量标准应用了此规则：“例如，将单个对象的5次检测（TP）计为1次正确检测和4次错误检测”
...
首先我们需要**按照其置信度对检测进行排序**，然后针对每个累积的检测计算精度和召回率，如下表所示（请注意，对于召回计算，分母项（“ Acc TP + Acc FN”或“所有基本事实”）恒定为15，因为无论检测到什么，GT盒都是恒定的）：

![img](https://github.com/rafaelpadilla/Object-Detection-Metrics/raw/master/aux_images/table_2_v2.png)

第二行的示例计算：P = TP /（TP + FP）= 1/2 = 0.5 且 R = TP /（TP + FN）= 1/15 = 0.066
...
$A P=0.0666+0.04446222+0.11428095+0.02026638=0.24560955$
