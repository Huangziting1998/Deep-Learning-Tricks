...
- 数据的标准化是**将数据按比例缩放，使之落入一个小的特定区间**：在某些比较和评价的指标处理中经常会用到，**去除数据的单位限制，将其转化为无量纲的纯数值，**便于不同单位或量级的指标能够进行比较和加权，其中最典型的就是数据的归一化处理，即将数据统一映射到[0,1]区间上；
- 数据归一化后， **更容易正确的收敛到最优解**、提升模型的精度；深度学习中数据归一化可以**防止模型梯度爆炸**；
...
- 积分图像和盒子滤波器结合，取代了高斯金字塔；
- 使用Harr小波计算特征点描述子，维度降低为64维；
