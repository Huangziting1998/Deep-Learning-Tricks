...
```
 ...
 - DropOut2d / DropConnect / 随机深度 / DropBlock（Dropout对卷积层的效果不好；block size控制大小最好在7x7，；keep prob在整个训练过程中从1逐渐衰减到指定阈值比较好）
 ...
 - 3x3 conv（有利于保持图像性质）
 - 卷积核权重初始化使用xavier（Tanh）或者He normal（ReLU） 
 - cv2要比os读取图片速度快
...
```
Mix up / Cut out / Random earse / CutMix / Mosaic
...
- 当显存小，batchsize不够时，可以使用梯度累积**变相增大batchsize**；
- weight在**不同模型之间交互**时候有好处；（动手学习深度学习v2）
...
**Droupout**由于在训练和测试时操作不同，所以**建议使用nn.Module实现**，它能够通过**model.eval**加以区分
...
- **nn.Sequential内部实现了forward函数**，因此可以不用写forward函数（或者继承nn.Module类的话，就要写出forward函数）；而nn.ModuleList则没有实现内部forward函数； -> 如果完全直接用 nn.Sequential，确实是可以的，但这么做的代价就是**失去了部分灵活性**，不能自己去定制 forward 函数里面的内容了;
...
**B x H x W，不涉及Channel，数据归一化方法**（批量归一化层：对于全连接，作用在特征维，对于卷积，作用在通道维）
...
- v4：残差连接，和ResNeXt特别像，唯一的区别是merge和1x1的位置；
