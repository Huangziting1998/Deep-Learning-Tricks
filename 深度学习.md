...
但是为什么使用FPN而不是FCN并没有给出理论上的解释，因此首先针对于涉烟数据集进行分析：数据集中烟支和烟条都为条形Style，条状的目标其长边通常远远大于短边，此时如果需要让感受野中心命中它，那么需要考虑的是其短边的长度; 然而检测它需要具备多大的感受野，是其长边决定的; 那么对网络的要求就是：既要命中多，又要感受野大；例如要求感受野中心的步长为8的同时, 感受野要达到200以上，FCN需要堆叠更多的block来得到大尺寸的感受野，这样使得FCN网络很深，不再具有速度优势， 而FPN可以将高层较大感受野信息引入到低层，达到上述要求，且不显著增加计算量，这证明了FPN style对于涉烟数据集检测的可行性；
...
- **逻辑回归**相对来说模型更简单，好理解，特别是**大规模线性分类**时比较方便；SVM的理解和优化相对来说复杂一些，**SVM转化为对偶问题后,分类只需要计算与少数几个支持向量的距离,**这个在进行复杂核函数计算时优势很明显,能够大大简化模型和计算；
- SVM不直接依赖数据分布，而LR则依赖，因为**SVM只与支持向量那几个点有关系，而LR和所有点都有关系**；
